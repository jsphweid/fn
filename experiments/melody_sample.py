...
from old_infer import magenta_utils
...
def generate(estimator, conditional_encoders, inputs):
    ...
    magenta_utils.LOGGER.info('Generating %d samples with format %s' % (FLAGS.num_samples, base_name))
    for i in range(FLAGS.num_samples):
        magenta_utils.LOGGER.info('Generating sample %d' % i)
        # Start the Estimator, loading from the specified checkpoint.
        input_fn = decoding.make_input_fn_from_generator(magenta_utils.melody_input_generator(
            inputs, FLAGS.decode_length))
        melody_conditioned_samples = estimator.predict(
            input_fn, checkpoint_path=FLAGS.model_path)

        # Generate sample events.
        magenta_utils.LOGGER.info('Generating sample.')
        sample_ids = next(melody_conditioned_samples)['outputs']

        # Decode to NoteSequence.
        magenta_utils.LOGGER.info('Decoding sample id')
        midi_filename = magenta_utils.decode(
            sample_ids,
            encoder=conditional_encoders['targets'])
        accompaniment_ns = magenta_utils.mm.midi_file_to_note_sequence(midi_filename)
        magenta_utils.mm.sequence_proto_to_midi_file(accompaniment_ns, base_name.replace('*', '%03d' % i))
...
def run():
    ...
    problem = magenta_utils.MelodyToPianoPerformanceProblem()
    melody_conditioned_encoders = problem.get_feature_encoders()
    melody_ns = magenta_utils.get_melody_ns(FLAGS.melody_path)
    ...
    # Create Estimator.
    magenta_utils.LOGGER.info('Loading model.')
...
def main(unused_argv):
    ...
    magenta_utils.LOGGER.set_verbosity(FLAGS.log)
    run()
